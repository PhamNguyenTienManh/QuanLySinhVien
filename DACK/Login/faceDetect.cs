    ...
    public class FaceDetect : Form
    {
        ...
        private void SaveImage(Rectangle face)
        {
            if (isEnable_SaveImage)
            {
                ...
                for (int i = 0; i < 30; i++)
                {
                    image.Resize(100, 100, Inter.Cubic).Save(Environment.CurrentDirectory + "\\Image\\" + ImageName + "_" + i.ToString() + ".jpg");
                }
            ...
            try
            {
                if (isTrained)
                {
                    ...
                    if (predictionResult.Label != -1 && predictionResult.Distance < distance && predictionResult.Distance > threshold)
                    {
                        ...
                        setPersonName = Names[predictionResult.Label].Replace(Environment.CurrentDirectory + "\\Image\\", "").Split('_')[0]; // Chỉ lấy phần tên người dùng
        ...
        public string DetectName()
        {
            ...
            if (array.Length != 0)
            {
                foreach (Rectangle rectangle in array)
                {
                    ...
                    {
                        return setPersonName.Split('_')[0]; // Chỉ trả về phần tên người dùng
                    }
                }
            }
            else
